...
Describe 'Get-PfbArrayConnectionPath - selector query keys (#64)' {
    ...
    It 'sends remote_ids when -RemoteId is used, and no other selector key' {
        Get-PfbArrayConnectionPath -RemoteId 'r-1' -Array $fakeArray
    ...
    It 'sends ids when -Id is used, and no remote_names' {
        Get-PfbArrayConnectionPath -Id 'conn-1','conn-2' -Array $fakeArray
    ...
    It 'composes -Id with -RemoteId and emits both plural keys' {
        Get-PfbArrayConnectionPath -Id 'conn-1' -RemoteId 'r-1' -Array $fakeArray
    ...
    It 'rejects -RemoteName together with -RemoteId at bind time, and makes no API call' {
        { Get-PfbArrayConnectionPath -RemoteName 'FB-B' -RemoteId 'r-1' -Array $fakeArray -ErrorAction Stop } |
    ...
    It 'puts no ValidateSet on the free-form selector -<Parameter>' -ForEach @(
        ...
        $attrs = (Get-Command Get-PfbArrayConnectionPath).Parameters[$Parameter].Attributes
