...
Describe 'Remove-PfbPolicyFileSystemReplicaLink - member selection is by ID only (#88)' {
    ...
    It 'publishes no MemberName parameter' {
        (Get-Command Remove-PfbPolicyFileSystemReplicaLink).Parameters.Keys | Should -Not -Contain 'MemberName'
    }

    It 'hides no MemberName alias on another parameter' {
        $aliases = (Get-Command Remove-PfbPolicyFileSystemReplicaLink).Parameters.Values | ForEach-Object { $_.Aliases }
        $aliases | Should -Not -Contain 'MemberName'
    }

    It 'fails at binding when -MemberName is supplied, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -PolicyName 'p1' -MemberName 'fs01' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
            Should -Throw
    ...
    It 'sends policy + member + remote qualifier on the wire' {
        Remove-PfbPolicyFileSystemReplicaLink -PolicyId 'pid1' -MemberId 'm1' -RemoteId 'ri1' `
            -Confirm:$false -Array $script:fakeArray

        Should -Invoke -ModuleName PureStorageFlashBladePowerShell Invoke-PfbApiRequest -Times 1 -Exactly -ParameterFilter {
            $Method -eq 'DELETE' -and $Endpoint -eq 'policies/file-system-replica-links' -and
    ...
    It 'sends remote_names when -RemoteName qualifies the removal' {
        Remove-PfbPolicyFileSystemReplicaLink -PolicyName 'p1' -MemberId 'm1' -RemoteName 'r1' `
            -Confirm:$false -Array $script:fakeArray
...
Describe 'Remove-PfbPolicyFileSystemReplicaLink - fails closed before any destructive request (#88)' {
    ...
    It 'refuses a policy-wide removal with no member, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -PolicyName 'p1' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
    ...
    It 'refuses an unscoped removal with no policy, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -MemberId 'm1' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
    ...
    It 'refuses a remote-only removal with no policy, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -RemoteId 'ri1' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
            Should -Throw '*A policy must be identified: supply -PolicyName or -PolicyId.*'
    ...
    It 'refuses both policy selectors together, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -PolicyName 'p1' -PolicyId 'pid1' -MemberId 'm1' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
    ...
    It 'refuses both remote selectors together, and deletes nothing' {
        { Remove-PfbPolicyFileSystemReplicaLink -PolicyName 'p1' -MemberId 'm1' -RemoteName 'r1' -RemoteId 'ri1' -Confirm:$false -Array $script:fakeArray -ErrorAction Stop } |
            Should -Throw '*mutually exclusive*'
...
Describe 'Remove-PfbPolicyFileSystemReplicaLink - endpoint declares no generic ids (#88)' {

    It 'publishes no <Param> parameter' -ForEach @(
        @{ Param = 'Id' }
        @{ Param = 'Name' }
    ) {
        (Get-Command Remove-PfbPolicyFileSystemReplicaLink).Parameters.Keys | Should -Not -Contain $Param
    }
}

Describe 'Remove-PfbPolicyFileSystemReplicaLink - help claims only what the endpoint declares (#88)' {
    ...
    It 'documents every published parameter' {
        $h = Get-Help Remove-PfbPolicyFileSystemReplicaLink -Full -ErrorAction Stop
        $h.Description.Text | Should -Not -BeNullOrEmpty

        $documented = @($h.parameters.parameter.name)
        $published = (Get-Command Remove-PfbPolicyFileSystemReplicaLink).Parameters.Keys |
            Where-Object {
